...
export class ReminderService {
   ...
   */
  private static deveEnviarEmail(
    ultimoEmail: Date | undefined,
    tentativa: number,
    diasDesdeUltimoEmail: number
  ): boolean {
   ...
   */
  static async verificarUsuariosSemEvento(): Promise<Usuario[]> {
    ...
    return usuarios.filter(usuario => {
      ...
      const deveEnviar = this.deveEnviarEmail(
        usuario.ultimoEmailReativacaoSemEvento,
        usuario.tentativaEmailReativacaoSemEvento || 0,
        diasDesdeCadastro >= 3 ? 7 : 3 // Primeiro email após 3 dias, reenvios a cada 7 dias
      );
   ...
   */
  static async verificarEventosSemParticipantes(): Promise<Grupo[]> {
    ...
    return eventos.filter(evento => {
      ...
      const deveEnviar = this.deveEnviarEmail(
        evento.ultimoEmailReativacaoSemParticipantes,
        evento.tentativaEmailReativacaoSemParticipantes || 0,
        diasDesdeCriacao >= 2 ? 7 : 2 // Primeiro email após 2 dias, reenvios a cada 7 dias
      );
   ...
   */
  static async verificarEventosSemDespesas(): Promise<Grupo[]> {
    const doisDiasAtras = new Date();
    doisDiasAtras.setDate(doisDiasAtras.getDate() - 2);
    doisDiasAtras.setHours(0, 0, 0, 0);
    ...
    const eventosSemDespesas: Grupo[] = [];
    ...
    for (const evento of eventosComParticipantes) {
      ...
      const temDespesasRecentes = despesas.some(
        (d) => d.criadoEm >= primeiroParticipanteData
      );

      if (!temDespesasRecentes) {
        // Verificar se deve enviar email baseado no tracking
        const diasDesdePrimeiroParticipante = Math.floor(
          (new Date().getTime() - primeiroParticipanteData.getTime()) / (1000 * 60 * 60 * 24)
        );
        
        const deveEnviar = this.deveEnviarEmail(
          evento.ultimoEmailReativacaoSemDespesas,
          evento.tentativaEmailReativacaoSemDespesas || 0,
          diasDesdePrimeiroParticipante >= 2 ? 7 : 2 // Primeiro email após 2 dias, reenvios a cada 7 dias
        );
   ...
   */
  static async enviarEmailsReativacao(): Promise<void> {
    try {
      ...
      for (const usuario of usuariosSemEvento) {
        try {
          const diasDesdeCadastro = Math.floor(
            (new Date().getTime() - usuario.criadoEm.getTime()) / (1000 * 60 * 60 * 24)
          );

          await EmailQueueService.adicionarEmailReativacaoSemEvento({
      ...
      for (const evento of eventosSemParticipantes) {
        try {
          ...
          const diasDesdeCriacao = Math.floor(
            (new Date().getTime() - evento.criadoEm.getTime()) / (1000 * 60 * 60 * 24)
          );
          ...
          await EmailQueueService.adicionarEmailReativacaoSemParticipantes({
      ...
      for (const evento of eventosSemDespesas) {
        try {
          ...
          const diasDesdeUltimaParticipacao = Math.floor(
            (new Date().getTime() - primeiroParticipanteData.getTime()) / (1000 * 60 * 60 * 24)
          );

          await EmailQueueService.adicionarEmailReativacaoSemDespesas({
            destinatario: organizador.email,
            nomeDestinatario: organizador.nome,
            eventoNome: evento.nome,
            eventoId: evento.id,
