...
export class AuthService {
  ...
  static async updateEmailPreferences(
    id: number,
    data: { receberEmails: boolean; emailOptOutReason?: string }
   ...
   */
  static gerarReferralCode(usuarioId: number): string {
    // Gerar código baseado no ID do usuário + hash simples
    // Formato: USER_[ID]_[hash_curto]
    const hash = crypto.createHash('md5').update(`${usuarioId}_${Date.now()}`).digest('hex').substring(0, 8).toUpperCase();
   ...
   */
  static async obterReferralCode(usuarioId: number): Promise<string> {
   ...
   */
  static parseReferralCode(code: string): { tipo: 'user' | 'evento' | 'share'; ids: number[] } | null {
    if (!code) return null;
