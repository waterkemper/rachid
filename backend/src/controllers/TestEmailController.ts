...
export class TestEmailController {
   ...
   */
  static async enviarBoasVindas(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      await EmailService.enviarEmailBoasVindas(email, nome, frontendUrl);
   ...
   * Requer: Autenticação + Role ADMIN
   */
  static async enviarNovaDespesa(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (process.env.NODE_ENV === 'production') {
        res.status(403).json({ error: 'Endpoint de teste não disponível em produção' });
        return;
      ...
      await EmailService.enviarEmailNovaDespesa({
        destinatario: email,
        nomeDestinatario,
        eventoNome,
   ...
   * Requer: Autenticação + Role ADMIN
   */
  static async enviarDespesaEditada(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (process.env.NODE_ENV === 'production') {
        res.status(403).json({ error: 'Endpoint de teste não disponível em produção' });
        return;
      ...
      await EmailService.enviarEmailDespesaEditada({
        destinatario: email,
        nomeDestinatario,
        eventoNome,
        eventoId: Number(eventoId),
   ...
   * Requer: Autenticação + Role ADMIN
   */
  static async enviarInclusaoEvento(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (process.env.NODE_ENV === 'production') {
        res.status(403).json({ error: 'Endpoint de teste não disponível em produção' });
        return;
      ...
      await EmailService.enviarEmailInclusaoEvento({
        destinatario: email,
        nomeDestinatario,
        eventoNome,
        eventoId: Number(eventoId),
   ...
   * Requer: Autenticação + Role ADMIN
   */
  static async enviarReativacaoSemEvento(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (process.env.NODE_ENV === 'production') {
        res.status(403).json({ error: 'Endpoint de teste não disponível em produção' });
        return;
      ...
      const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
      await EmailService.enviarEmailReativacaoSemEvento({
        destinatario: email,
        nomeDestinatario,
   ...
   * Requer: Autenticação + Role ADMIN
   */
  static async enviarReativacaoSemParticipantes(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (process.env.NODE_ENV === 'production') {
        res.status(403).json({ error: 'Endpoint de teste não disponível em produção' });
        return;
      ...
      const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
      await EmailService.enviarEmailReativacaoSemParticipantes({
        destinatario: email,
        nomeDestinatario,
        eventoNome,
        eventoId: Number(eventoId),
   ...
   * Requer: Autenticação + Role ADMIN
   */
  static async enviarReativacaoSemDespesas(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (process.env.NODE_ENV === 'production') {
        res.status(403).json({ error: 'Endpoint de teste não disponível em produção' });
        return;
      ...
      const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
      await EmailService.enviarEmailReativacaoSemDespesas({
        destinatario: email,
        nomeDestinatario,
        eventoNome,
        eventoId: Number(eventoId),
   ...
   * Requer: Autenticação + Role ADMIN
   */
  static async listarTipos(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (process.env.NODE_ENV === 'production') {
        res.status(403).json({ error: 'Endpoint de teste não disponível em produção' });
        return;
