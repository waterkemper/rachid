...
async function requirePro(req, res, next) {
    try {
        ...
        if (subscription.trialEnd && new Date(subscription.trialEnd) < now) {
            ...
            await SubscriptionService_1.SubscriptionService.checkAndHandleExpiredSubscription(subscription);
            // Reload subscription to get updated status
            const updatedSubscription = await SubscriptionService_1.SubscriptionService.getSubscription(usuarioId);
